    ...
    >
      ...
      >
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`absolute inset-0 backface-hidden rounded-[2.5rem] border-2 bg-slate-900/60 backdrop-blur-2xl p-6 flex transition-all duration-500
            ...
            ${isVertical ? 'flex-row gap-6' : 'flex-col'}`}
        >
          {isVertical && scene.asset && (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="w-1/3 h-full relative rounded-2xl overflow-hidden group/img border border-white/10 shrink-0">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-all duration-300 flex items-center justify-center backdrop-blur-sm">
                ...
                }} className="p-2.5 bg-red-500 hover:bg-red-600 rounded-xl text-white shadow-xl">
                  <Trash2 size={16} />
                </button>
              </div>
            </motion.div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 relative group/textarea min-h-0">
              <textarea
              ...
              <div className="absolute top-3 right-3 opacity-0 group-hover/textarea:opacity-100 transition-opacity">
                 <Copy size={16} className="text-slate-500 cursor-pointer hover:text-emerald-400" onClick={() => navigator.clipboard.writeText(showTranslateImage ? (scene.translatedImageText||"") : scene.imageText)} />
              </div>
            </div>

            {!isVertical && scene.asset && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 relative rounded-xl overflow-hidden h-28 group/img border border-white/10 shrink-0">
                <img src={assetUrl || scene.asset} alt="Ref" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-all duration-300 flex items-center justify-center backdrop-blur-sm">
                  <button onClick={async () => {
                    if (scene.asset) await AssetManager.deleteAsset(scene.asset);
        ...
        <div
          onDragOver={(e) => e.preventDefault()}
          onDrop={handleDrop}
          className={`absolute inset-0 backface-hidden rotate-y-180 rounded-[2.5rem] border-2 bg-slate-900/80 backdrop-blur-2xl p-6 flex transition-all duration-500
        ...
        >
          {isVertical && scene.asset && (
            <div className="w-1/3 h-full relative rounded-2xl overflow-hidden group/img border border-white/10 shrink-0">
              <img src={assetUrl || scene.asset} alt="Ref" className="w-full h-full object-cover" />
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-1 relative group/textarea min-h-0">
              ...
              />
              <div className="absolute top-3 right-3 opacity-0 group-hover/textarea:opacity-100 transition-opacity">
                 <Copy size={16} className="text-slate-500 cursor-pointer hover:text-violet-400" onClick={() => navigator.clipboard.writeText(showTranslateVideo ? (scene.translatedVideoText||"") : scene.videoText)} />
              </div>
            </div>

            <div className={`mt-4 flex items-center justify-center border-2 border-dashed border-violet-500/20 rounded-xl bg-violet-500/5 group/preview overflow-hidden relative shrink-0 ${isVertical ? 'h-16' : 'h-24'}`}>
